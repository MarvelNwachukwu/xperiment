...
`output/messages.json` is produced by the external writer AI — it is not generated by any script in this repo. The shape is:
...
`tone` is `"cold"` or `"warm"`. The file is gitignored under `output/`.
